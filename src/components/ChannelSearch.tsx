    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        <input
          ...
          placeholder="채널명 검색..."
          className="flex-1 min-w-0 px-3 sm:px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-sm sm:text-base text-white placeholder-gray-400 focus:outline-none focus:border-blue-500"
        />
        <button
          onClick={handleSearch}
          disabled={isLoading || !keyword.trim()}
          className="px-3 sm:px-4 py-2 bg-blue-600 text-white text-sm sm:text-base rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex-shrink-0"
        >
          {isLoading ? '...' : '검색'}
        </button>
      </div>
